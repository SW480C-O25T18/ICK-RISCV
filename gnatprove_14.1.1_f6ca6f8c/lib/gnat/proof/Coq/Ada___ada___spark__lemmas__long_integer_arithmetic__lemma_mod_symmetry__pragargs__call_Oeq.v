...
Axiom length'def :
  ...
  ((x <= y)%Z -> ((length x y) = ((y - x)%Z + 1%Z)%Z)) /\ (* non escaped EOL for cpp parsing *)
  (~ (x <= y)%Z -> ((length x y) = 0%Z)).

Axiom long_integer : Type.
Parameter long_integer_WhyType : WhyType long_integer.
Existing Instance long_integer_WhyType.

Parameter long_integer'int: long_integer -> Numbers.BinNums.Z.

Axiom long_integer'axiom :
  forall (i:long_integer),
  ((-9223372036854775808%Z)%Z <= (long_integer'int i))%Z /\ (* non escaped EOL for cpp parsing *)
  ((long_integer'int i) <= 9223372036854775807%Z)%Z.
...
Parameter user_eq: long_integer -> long_integer -> Init.Datatypes.bool.

Parameter dummy: long_integer.

(* Why3 assumption *)
Inductive long_integer__ref :=
  | long_integer__ref'mk : long_integer -> long_integer__ref.
Axiom long_integer__ref_WhyType : WhyType long_integer__ref.
Existing Instance long_integer__ref_WhyType.

(* Why3 assumption *)
Definition long_integer__content (v:long_integer__ref) : long_integer :=
  match v with
  | long_integer__ref'mk x => x
  end.

(* Why3 assumption *)
Definition long_integer__ref_long_integer__content__projection
    (a:long_integer__ref) : long_integer :=
  long_integer__content a.

(* Why3 assumption *)
Definition dynamic_invariant (temp___expr_25:Numbers.BinNums.Z)
    (temp___is_init_21:Init.Datatypes.bool)
    (temp___skip_constant_22:Init.Datatypes.bool)
    (temp___do_toplevel_23:Init.Datatypes.bool)
    (temp___do_typ_inv_24:Init.Datatypes.bool) : Prop :=
  (temp___is_init_21 = Init.Datatypes.true) \/
  ((-9223372036854775808%Z)%Z <= 9223372036854775807%Z)%Z ->
