...
Axiom fliteral_axiom3 : t'isFinite fliteral3 /\ ((t'real fliteral3) = 0%R).
...
Theorem def'vc :
  ...
  (((le (neg fliteral2) val1 /\ le val1 fl_last_sqrt) /\ (* non escaped EOL for cpp parsing *)
   ...
   le fliteral3 val3 /\ le val3 fl_last_sqrt) /\ (* non escaped EOL for cpp parsing *)
  le val1 val2 -> le (mul RNE val1 val3) (mul RNE val2 val3).
Proof.
...
(* see float64_mul_common.prf *)
Require Import Fourier.


assert (forall x y, x <= y -> x <= 1+y) as g.
{ intros.
  apply Rle_trans with (r2:=y); auto.
  rewrite Rplus_comm.
  left; apply Rlt_plus_1. }

assert (t'real fliteral2 * t'real fliteral2 <= 9007199254740991 * ) as l2square_le_maxfloat.
{
 Require Import Flocq.Core.Core.
  destruct fliteral_axiom2 as [fliteral2_finite l_real2].
  rewrite l_real2. ring_simplify.
  now apply (IZR_le  ). }

(* file shared by the two lemmas dealing 32/64-bit mult monotonicity *)
...
{ rewrite <- h2 in *.
  ...
  rewrite Abs.Abs_le. rewrite l_real3 in *; split; auto. apply Rle_trans with (0/10); fourier.
}
...
(* deduce the projections of val1 * val3 and val2 * val3 and that they
   are finite *)
destruct (mul_finite RNE _ _ h3 h5 no_ov_v1v3) as (v1v3_finite, v1v3_real).
destruct (mul_finite RNE _ _ h4 h5 no_ov_v2v3) as (v2v3_finite, v2v3_real).
...
rewrite l_real3 in h10; fourier.
fourier.
