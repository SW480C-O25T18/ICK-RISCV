...
Axiom nat'axiom :
  forall (i:nat), (0%Z <= (nat'int i))%Z /\ ((nat'int i) <= 2147483647%Z)%Z.
...
Axiom bool_eq'def1 :
  forall (x:Numbers.BinNums.Z) (y:Numbers.BinNums.Z),
  ((x = y) -> ((bool_eq3 x y) = Init.Datatypes.true)) /\ (* non escaped EOL for cpp parsing *)
  (~ (x = y) -> ((bool_eq3 x y) = Init.Datatypes.false)).
...
Parameter user_eq2: nat -> nat -> Init.Datatypes.bool.

Parameter dummy2: nat.
...
(* Why3 assumption *)
Definition dynamic_invariant1 (temp___expr_166:Numbers.BinNums.Z)
    (temp___is_init_162:Init.Datatypes.bool)
    (temp___skip_constant_163:Init.Datatypes.bool)
    (temp___do_toplevel_164:Init.Datatypes.bool)
    (temp___do_typ_inv_165:Init.Datatypes.bool) : Prop :=
  (temp___is_init_162 = Init.Datatypes.true) \/ (0%Z <= 2147483647%Z)%Z ->
  in_range1 temp___expr_166.

(* Why3 assumption *)
Definition dynamic_invariant2 (temp___expr_188:Numbers.BinNums.Z)
...
Theorem def'vc :
  ...
  ((bool_le (int.Power.power val1 exp) (int.Power.power val2 exp)) =
   Init.Datatypes.true).
Proof.
